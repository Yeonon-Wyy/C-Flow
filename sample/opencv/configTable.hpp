...
bool watermark(std::shared_ptr<FrameRequest> request) {
  ...
  // std::this_thread::sleep_until(vtf::utils::TimeUtil::awake_time(24) -
  // durationTime);
  return true;
}
    ...
    {.maxProcessingSize = 8,
     ...
     .pipeNodeCreateInfos =
         {{.id = 1,
           ...
           .processCallback =
               std::bind(&dnnfacedetect::detect, dnnfacedetect::getInstance(),
                         std::placeholders::_1),
           .configProgress =
               std::bind(&dnnfacedetect::config, dnnfacedetect::getInstance())},
          {.id = 2,
           .name = "FDVideoNode",
