...
bool imageShowResultCallback(std::shared_ptr<FrameRequest> request)
{
    ...
    if (request->scenario() == CVTestScenario::VIDEO)
    {
        ...
        {
            CFLOW_LOGD("result face roi [{0},{1},{2},{3}]", face.x, face.y, face.width, face.height);

            rectangle(*frame, Point(face.x, face.y), Point(face.x + face.width, face.y + face.height), Scalar(0, 0, 255), 1, 8);
        }
    }
